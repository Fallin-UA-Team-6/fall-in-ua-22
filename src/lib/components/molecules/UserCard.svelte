...
<section class="space-y-4 h-auto inline min-h-0">
    <div class="h-24 flex justify-between items-start">
        ...
        <div class="flex flex-col gap-2 items-end justify-between h-full">
            <span class="timestamp">Last Check in:<br/> {member?.latestCheckin?.timestamp ? new Date(member?.latestCheckin?.timestamp).toLocaleDateString() : "This user has not yet checked in"}</span>
